...
use super::common::{
    download_file, extract_zip, find_column_index, find_first_matching_file, normalize_postcode,
    open_source_db, to_i64, with_temp_dir,
};
...
fn resolve_rofrs_csv_path(temp: &tempfile::TempDir) -> Result<Option<PathBuf>> {
    if let Ok(local_csv) = env::var("FLOOD_ROFRS_CSV_PATH") {
        return Ok(Some(PathBuf::from(local_csv)));
    }
    ...
    if download_file(&url, &zip_path).is_err() {
        return Ok(None);
    }
...
fn discover_rofrs_csv(root: &Path) -> Result<Option<PathBuf>> {
    find_first_matching_file(root, &|path| {
        path.extension()
